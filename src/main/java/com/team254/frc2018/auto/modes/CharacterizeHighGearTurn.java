...
public class CharacterizeHighGearTurn extends AutoModeBase {
    @Override
    protected void routine() throws AutoModeEndedException {
        List<double[]> velocityData = new ArrayList<>();
        List<double[]> accelerationData = new ArrayList<>();

        runAction(new CollectDriveData(velocityData, 0.02, 0.25, true, false, true));
        runAction(new WaitAction(2));
        runAction(new CollectDriveData(velocityData, 0.5, 0.75, true, false, true));
