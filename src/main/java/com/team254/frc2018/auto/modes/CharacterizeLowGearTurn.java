...
public class CharacterizeLowGearTurn extends AutoModeBase {
    @Override
    protected void routine() throws AutoModeEndedException {
        List<double[]> velocityData = new ArrayList<>();
        List<double[]> accelerationData = new ArrayList<>();

        runAction(new CollectDriveData(velocityData, 0.02, 0.25, false, false, true));
        runAction(new WaitAction(2));
        runAction(new CollectDriveData(velocityData, 0.5, 0.75, false, false, true));
